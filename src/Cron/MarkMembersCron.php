...
class MarkMembersCron {
  ...
  public function __invoke(): void
  {
    ...
    if ($limit > 0) {
      ...
      $members = $db->prepare("SELECT * FROM `tl_member` WHERE `lastLogin` > 0 AND `lastLogin` < ? AND (`marked_for_removal` IS NULL OR `marked_for_removal` = 0) ORDER BY `currentLogin` ASC, `dateAdded` ASC, `id` ASC")
        ->limit($limit)
